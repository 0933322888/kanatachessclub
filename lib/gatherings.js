...
export function getDefaultGatheringSlots(count = 5, startDate = new Date(), clubId = null) {
    const { gatheringDay, scheduleType, biweeklyAnchorDate } = getSiteConfig(clubId);
    const start = new Date(startDate);
    start.setHours(0, 0, 0, 0);

    if (scheduleType === 'biweekly' && biweeklyAnchorDate) {
        const [year, month, day] = biweeklyAnchorDate.split('-').map(Number);
        ...
        let nextGathering = new Date(anchor);
        if (nextGathering < start) {
            const daysSinceAnchor = Math.floor((start - nextGathering) / msPerDay);
            const intervalsToAdvance = Math.ceil(daysSinceAnchor / intervalDays);
            nextGathering.setDate(nextGathering.getDate() + (intervalsToAdvance * intervalDays));
        }

        // Guard against any DST/date math edge cases.
        while (nextGathering < start) {
            nextGathering.setDate(nextGathering.getDate() + intervalDays);
        }

        while (slots.length < count) {
            slots.push(new Date(nextGathering));
            nextGathering.setDate(nextGathering.getDate() + intervalDays);
        }
