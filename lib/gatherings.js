 ...
 */
export function getDefaultGatheringSlots(count = 5, startDate = new Date()) {
 ...
 */
export async function getUpcomingGatherings(count = 5) {
    await connectDB();
    ...
    while (finalGatherings.length < count) {
        const slots = getDefaultGatheringSlots(slotsToFetch, currentStartDate);
